...
Today, I am using the *Conductivity* and *Depth* datasets from our class exercise.  
...
#| warning: false
#| message: false
#| label: fig-cond-depth
#| fig-cap: "Average Depth, Salinity, and Temperature Over Time"
#| fig-align: "center"
#| out-width: "80%"
...
| Time                | Depth (m) | Salinity (PSU) | Temperature (°C) |
|:--------------------|:----------:|:---------------:|:-----------------:|
| _Start (11:30 UTC)_ | 0.25       | 34.0            | 29.2             |
| _Midpoint (12:00 UTC)_ | 0.30       | 33.6            | 29.3             |
| _End (12:30 UTC)_   | **0.32**   | **33.5**        | **30.0**         |

```{r}
#| warning: false
#| message: false
#| label: tbl-time-change
#| tbl-cap: "Changes in Average Depth, Salinity, and Temperature at Key Time Points"
...
# making it easy to compare patterns seen in the line plot.

tibble(
  Time_Point = c("Start (11:30 UTC)", "Midpoint (12:00 UTC)", "End (12:30 UTC)"),
  Depth_m = c(0.25, 0.30, 0.32),
  Salinity_PSU = c(34.0, 33.6, 33.5),
  Temperature_C = c(29.2, 29.3, 30.0)
) %>%
  kbl(
    caption = "Average depth, salinity, and temperature measured at key time intervals during the one-hour period.",  # short note that explains what the table shows
    col.names = c("Time Point", "Depth (m)", "Salinity (PSU)", "Temperature (°C)"),  # rename columns
    align = "c"  # center all text
  ) %>%
  kable_classic(full_width = FALSE, html_font = "Arial") %>%
  kable_styling(
    bootstrap_options = c("striped", "hover", "condensed", "responsive"),  # add light stripes and compact layout
    position = "center",
    full_width = FALSE,
    font_size = 14
  ) %>%
  row_spec(0, bold = TRUE, color = "white", background = "#2a5674") %>%   # deep ocean blue header
  row_spec(3, bold = TRUE, color = "white", background = "#b35c44") %>%   # warm terracotta for end time
...
@tbl-time-change shows how the three variables change over the one-hour period.
Between 11:30 and 12:30 UTC, temperature clearly rises, while salinity slightly decreases and depth becomes a bit higher.
This table helps confirm the same patterns shown in the figure — especially the steady increase in temperature toward the end.
